...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR('Przesyłka nie jest kierowana do paczkomatu', 16, 1);
            RETURN;
        END
        ...
        UPDATE SkrytkiPaczkomatow SET Status = 'Zajęta' WHERE SkrytkaID = @SkrytkaID;
        ...
        EXEC sp_WyslijPowiadomienie @OdbiorcaID, @PrzesylkaID, 'Dostawa', 'SMS';
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(CASE WHEN Status = 'Zajęta' THEN 1 END) AS ZajeteSkrytki
    FROM SkrytkiPaczkomatow
    WHERE DroppointID = @DroppointID
    GROUP BY Gabaryt
    ORDER BY Gabaryt;
