...
CREATE SEQUENCE SEQ_FAKT_TRANS START WITH 1 INCREMENT BY 1;
...
CREATE INDEX IDX_STAT_DATA ON STATYSTYKI_PUNKTOW_ODBIORU(DATA_ANALIZY);
CREATE INDEX IDX_STAT_PUNKT ON STATYSTYKI_PUNKTOW_ODBIORU(DROP_POINT_ID);
...
CREATE INDEX IDX_WYD_KURIER ON WYDAJNOSC_KURIEROW(KURIER_ID);
CREATE INDEX IDX_WYD_OKRES ON WYDAJNOSC_KURIEROW(OKRES_ROZLICZENIOWY);
...
CREATE INDEX IDX_WYD_PRAC ON WYDAJNOSC_PRACOWNIKOW_SORTOWNI(PRACOWNIK_ID);
CREATE INDEX IDX_WYD_SORT ON WYDAJNOSC_PRACOWNIKOW_SORTOWNI(SORTOWNIA_ID);
...
CREATE TABLE OCENY_PRACOWNIKOW (
    ...
    OCENIAJACY_ID INTEGER,
    DATA_UTWORZENIA TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    STATUS_OCENY VARCHAR2(30) DEFAULT 'WSTEPNA'
);

CREATE INDEX IDX_OCENA_PRAC ON OCENY_PRACOWNIKOW(PRACOWNIK_ID);
CREATE INDEX IDX_OCENA_OKRES ON OCENY_PRACOWNIKOW(OKRES_OCENY);
...
CREATE INDEX IDX_KOSZT_MIESIAC ON KOSZTY_OPERACYJNE(MIESIAC);
CREATE INDEX IDX_KOSZT_KAT ON KOSZTY_OPERACYJNE(KATEGORIA_KOSZTOW);
...
CREATE INDEX IDX_ZWROT_MIESIAC ON ANALIZA_ZWROTOW(MIESIAC);
...
CREATE INDEX IDX_DIM_DATA ON DIM_CZAS(DATA_PELNA);
CREATE INDEX IDX_DIM_ROK_MIESIAC ON DIM_CZAS(ROK, MIESIAC);
...
CREATE INDEX IDX_FAKT_DATA ON FAKT_TRANSAKCJE(DATA_TRANSAKCJI) LOCAL;
CREATE INDEX IDX_FAKT_NADAWCA ON FAKT_TRANSAKCJE(KLIENT_NADAWCA_ID) LOCAL;
